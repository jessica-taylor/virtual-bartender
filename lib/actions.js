...
/*allCriteria.push(['is', 'more_strong']);
allCriteria.push(['is' ,'less_strong']);
allCriteria.push(['is', 'more_sweet']);
allCriteria.push(['is' ,'less_sweet']);*/
...
function convertSubaction(suba, drink) {
  ...
  if (suba[1][1].indexOf('more_') != -1) {
    var prop = suba[1][1].substring(5);
    if (!drink) {
      return null;
    } else {
      suba[1] = ['>', prop, drink.computeAdjective(prop)];
    }
  } else if (suba[1][1].indexOf('less_') != -1) {
    if (!drink) {
      return null;
    } else {
      var prop = suba[1][1].substring(5);
