...
Bartender.prototype.handleInput = function(speech) {
  ...
  if (newCriteria.length == 0) {
    ...
    var criteriaText = _.map(self.criteria, function(c) { return c[1]; }).join(', ');
    sentence = 'You want something ' + criteriaText + '.';
    if (drinksSatisfying.length > 0) {
      self.suggestedDrink = drinksSatisfying[0];
      sentence += '  How about a ' + self.suggestedDrink.name + '?';
    } else {
      sentence += "  I don't know about any drink like that.";
      self.criteria = [];
    }
  }
