...
var HmmInference = require('./hmm_inference');
...
Bartender.prototype.handleInput = function(speech) {
  var self = this;
  self.rec.stop();
  self.emitter.emit('speech', 'user', _.max(speech, function(s) { return s.confidence; }).transcript);
  var observation = Matching.getObservation(speech);
  self.observations.push(observation);
  var stateDistr = HmmInference.inferState(self.observations, self.actions);
  console.log('stateDistr', stateDistr);
  var bestAction = Actions.bestAction(stateDistr, self.criteria, self.suggestedDrink);
