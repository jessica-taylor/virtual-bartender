...
var allProperties = ['strong', 'fruity', 'sweet', 'spicy', 'sour',
    'nutty','creamy','hot','iced', 'cold', 'flaming', 'blended', 'more_strong',
    'less_strong', 'more_sweet', 'less_sweet']

for (var ingredient in ingredientsJSON) {
  allProperties.push('has_' + ingredient);
}
...
Drink.prototype.getPropertyValue = function(property) {
  ...
  switch(property) {
    ...
    default:
      if (property.indexOf('has_') == 0) {
        var ingredientDesired = property.substring(4);
        for (var i = 0; i < self.ingredients.length; i++) {
          if (self.ingredients[i][0].indexOf(ingredientDesired) >= 0) {
            return true;
          }
        }
      }
